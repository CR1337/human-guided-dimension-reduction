...
export default {
    ...
    props: [
        'datapoints', 'k', 'distanceMetric',
        'coloring', 'metrics', 'labels'
    ],
    ...
    data() {
        return {
            ...
            hoveredFill: [128, 128, 128],
            selectedFill: [255, 255, 255],
    ...
    methods: {
        ...
        colorFromScaleInterpolated(p5, t) {
        ...
        getFill(datapointIndex) {
            if (this.coloring == "averageLocalError" && this.metrics != null) {
                ...
                return this.labelColors[this.datapoints[datapointIndex].label];
            }
        },
        ...
        drawPoints(p5) {
            ...
            for (const [i, datapoint] of this.datapoints.entries()) {
                if (this.selectedPointIndex == i) {
                    ...
                    selectedPointData = {
                        ...
                        stroke: (datapoint.is_landmark) ? this.landmarkStroke : null,
                        size: (datapoint.is_landmark) ? this.landmarkSize : this.pointSize,
                        shape: (datapoint.is_landmark) ? 'square' : 'circle'
                    };
                } else if (this.hoveredPointIndex == i) {
                    hoveredPointData = {
            ...
            for (const point of pointData) {
                this.drawPoint(p5, point.position[0], point.position[1], point.size, point.fill, point.stroke, point.shape);
            }

            for (const point of landmarkData) {
                this.drawPoint(p5, point.position[0], point.position[1], point.size, point.fill, point.stroke, point.shape);
            }

            if (hoveredPointData != null) {
                this.drawPoint(p5, hoveredPointData.position[0], hoveredPointData.position[1], hoveredPointData.size, hoveredPointData.fill, hoveredPointData.stroke, hoveredPointData.shape);
            }

            if (selectedPointData != null) {
                this.drawPoint(p5, selectedPointData.position[0], selectedPointData.position[1], selectedPointData.size, selectedPointData.fill, selectedPointData.stroke, selectedPointData.shape);
            }
        },
            ...
            switch(this.coloring) {
                case "label":
                    for (let i = 0; i < this.labels.length; ++i) {
                        p5.fill(this.labelColors[i]);
                        p5.rect(this.legendX, this.legendY + i * this.legendHeight, this.legendHeight);
                        p5.fill(255);
                        p5.text(this.labels[i], this.legendX + this.legendHeight + 4, this.legendY + i * this.legendHeight + 4);
                    }
                    break;
                ...
                case "averageLocalError":
                    for (let i = 0; i < this.legendWidth; ++i) {
                        ...
                        const c = this.colorFromScaleInterpolated(p5, t);
                        p5.fill(c);
